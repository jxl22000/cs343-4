...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    """
    foodHouseMarginal = inference.inferenceByVariableElimination(bayesNet, 
            [FOOD_HOUSE_VAR], evidence, eliminationOrder)
    maxProb = -1
    mostLikelyPos = None
    for assignment in foodHouseMarginal.getAllPossibleAssignmentDicts():
        prob = foodHouseMarginal.getProbability(assignment)
        if prob > maxProb:
            maxProb = prob
            mostLikelyPos = assignment[FOOD_HOUSE_VAR]
    return {FOOD_HOUSE_VAR: mostLikelyPos}
...
class VPIAgent(BayesAgent):
    ...
    def computeEnterValues(self, evidence, eliminationOrder):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        jointFactor = inference.inferenceByVariableElimination(self.bayesNet, 
                [FOOD_HOUSE_VAR, GHOST_HOUSE_VAR], evidence, eliminationOrder)
        cases = [(TOP_LEFT_VAL,  TOP_RIGHT_VAL), (TOP_RIGHT_VAL, TOP_LEFT_VAL)]
        for foodVal, ghostVal in cases:
            assign = dict(evidence)
            assign.update({FOOD_HOUSE_VAR: foodVal, GHOST_HOUSE_VAR: ghostVal})
            prob = jointFactor.getProbability(assign)
            if foodVal == TOP_LEFT_VAL:
                leftExpectedValue += prob * WON_GAME_REWARD
            else:
                leftExpectedValue += prob * GHOST_COLLISION_REWARD
            if foodVal == TOP_RIGHT_VAL:
    ...
    def computeExploreValue(self, evidence, enterEliminationOrder):
        ...
        for prob, outcomeEvidence in outcomes:
